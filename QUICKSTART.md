...
主要参数：
- `--mode`: 实验模式
  - `all`: 运行所有实验（默认）- 6个渐进式消融实验
  - `baseline`: 仅基线模型
  - `uniform`: 仅统一50%剪枝实验
  - `layer_adaptive`: 仅层级自适应压缩实验
  - `adaptive_importance`: 层级自适应+重要性评估
  - `adaptive_progressive`: 层级自适应+重要性评估+渐进式训练
  - `all_innovations`: 所有创新方法+量化
...
本项目采用渐进式消融实验设计，每个实验在前一个基础上添加一个创新：

1. **实验1 - 基线**：微调的BERT-base模型
2. **实验2 - 统一50%剪枝**：传统统一剪枝方法
3. **实验3 - 层级自适应**：创新1 - 不同层不同稀疏度
4. **实验4 - +重要性评估**：创新2 - 基于梯度的注意力头剪枝
5. **实验5 - +渐进式训练**：创新3 - 多阶段学习率调整
6. **实验6 - +动态量化**：完整方案 - 添加INT8量化
